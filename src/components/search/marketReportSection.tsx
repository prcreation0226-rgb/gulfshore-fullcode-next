...
export default function MarketReportSection({ city, community }: MarketReportSectionProps) {
	...
	if (loading) {
		return (
			<div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-6 my-6 animate-pulse">
				<div className="h-6 w-48 bg-gray-200 dark:bg-zinc-800 rounded mb-4"></div>
				<div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
					{[1, 2, 3, 4].map((i) => (
						<div key={i} className="h-20 bg-gray-100 dark:bg-zinc-800/50 rounded-xl"></div>
					))}
				</div>
			</div>
		);
	...
	return (
		<div className="bg-white dark:bg-zinc-900 rounded-2xl border border-gray-100 dark:border-zinc-800 p-6 shadow-sm my-8">
			<div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-6 pb-4 border-b border-gray-100 dark:border-zinc-800">
				...
				<span className="text-[11px] font-medium text-gray-400 bg-gray-50 dark:bg-zinc-800 px-3 py-1 rounded-full self-start sm:self-auto">
			...
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
				{/* Active Listings */}
				<div className="p-4 rounded-xl bg-gray-50 dark:bg-zinc-800/50 border border-gray-100 dark:border-zinc-800/80">
				...
				<div>
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
						{report.propertyTypes.map((item, idx) => (
							<div
								key={idx}
								className="p-3 rounded-lg bg-gray-50/70 dark:bg-zinc-800/30 border border-gray-100 dark:border-zinc-800 flex justify-between items-center text-xs">
