...
export async function POST(req: NextRequest) {
	try {
		...
		console.log("[Resend Inbound Webhook payload received]:", JSON.stringify(payload, null, 2));
		...
		if (!textBody && data.email_id) {
			console.log(`[Resend Webhook] Text is missing. Fetching full INBOUND email by ID: ${data.email_id}...`);
			try {
				let emailResponse;
				...
				console.error("[Resend Webhook] Error fetching inbound email body:", err);
			}
		}
		...
		console.log(`[Resend Webhook Parsed Data] From: ${fromEmail}, Subject: ${subject}, Text length: ${cleanTextBody?.length}`);

		if (!fromEmail || !cleanTextBody) {
			console.error("[Resend Webhook Error] Missing fields. Parsed as:", { fromEmail, subject, text: cleanTextBody ? 'Present' : 'Missing' });
		...
		console.log(`[Resend Webhook] Clean email: ${cleanEmail}`);
		...
		// 3. Generate AI Response using tools
		// @ts-ignore
...
on behalf of Dimitri Schwarz`,
		...
		// 4. Save AI's Reply to AI Chat History
		await prisma.aIChatHistory.create({
			data: {
				leadId: lead.id,
				channel: "email",
		...
		// 5. Send the Email back to the user via Resend
